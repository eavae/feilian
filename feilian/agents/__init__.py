...
#     PENDING = "pending"  # 新生成任务时的状态
#     SUCCEED = "succeed"  # 任务完成时的状态
#     FAILED = "failed"  # 任务测试失败时的状态
#     REWORKED = "reworked"  # 任务重新处理时的状态
...
class Operator(TypedDict):
    ...
    content_type: ContentTypes = ContentTypes.LAYOUT
...
def get_tree(snippet: Snippet, compact: bool = True) -> etree._Element:
    ...
    xpaths = deduplicate_xpath([op["xpath"] for op in prune_ops])
    for xpath in xpaths:
    ...
    xpaths = deduplicate_xpath([op["xpath"] for op in extract_ops])
...
def _create_table_extraction_chain():
    ...
    return (
        TABLE_EXTRACTION_PROMPT_CN.partial(chat_history=TABLE_EXTRACTION_PROMPT_HISTORY)
        | llm
    )
...
def merge_operations(operations: List[Operator]) -> List[Operator]:
    ...
    for k, group_ops in class_group.items():
        xpaths = deduplicate_xpath([o["xpath"] for o in group_ops])
        ops += [
            {
                "id": len(ops),
                "xpath": xpath,
